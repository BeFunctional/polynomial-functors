...
iType_ ii g (Sigma_ f s)      = do
  cType_ ii g f VStar_
...
iType_ ii g (PolyElim_ m f c) =
  do cType_ ii g m (VPi_ VPoly_ (\_ -> VStar_)) -- check the motive
     ...
     cType_ ii g c VPoly_ -- check the argument
     -- check the elimination function whose type depends on both arguments
     cType_ ii g f (VPi_ VStar_ (\shapes ->
                    VPi_ (VPi_ shapes (const VStar_)) (\positions ->
                    mVal `vapp_` VMkPoly_ shapes positions)))
     let cVal = cEval_ c (fst g, []) -- quote the container
     return (mVal `vapp_` cVal)
...
cType_ ii g (MkPoly_ x f) VPoly_ =
  do cType_ ii g x VStar_ -- check if the first argument is a type
     let xVal = cEval_ x (fst g, [])
     cType_ ii g f (VPi_ xVal (const VStar_)) -- check if the second argument is a Π
